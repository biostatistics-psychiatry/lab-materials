...
phq9_fit <- logistic_reg() |>
  ...
  fit(___ ~ ___ + ___, data = df_clean)

# Make predictions
phq9_pred <- predict(phq9_fit, new_data = df_clean) |>
  bind_cols(df_clean)
...
phq9_fit <- logistic_reg() |> #<1>
  ...
  fit(phq9_post_bin ~ phq9_screen + group, data = df_clean) #<4>

# Make predictions
phq9_pred <- predict(phq9_fit, new_data = df_clean) |> #<5>
  bind_cols(df_clean)

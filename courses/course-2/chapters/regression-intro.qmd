...
# we also rename the "group" variable, otherwise it causes issues with the `marginaleffects` package
...
head(residuals(mod_lm)) # returns the residuals (observed value - predicted value)
```

...
# Syntax of Intercept-Only Models in R
...
There are *no predictors*, so the model formula in R is simply:

```{r, echo=FALSE}
#| label: formula-intercept-only
lsas_post ~ 1
```
...
$$\mathbb{E}(LSAS_{post}) = \beta_0$$

Where $\mathbb{E}$ refers to the expected value (mean), and $\beta_0$ denotes the intercept. Sometimes, you may see this same model described using this formula:

$$LSAS_{post_i} = \beta_0 + \epsilon_i $$

That describes the same model, but highlights that the individual observations, denoted by $i$, are a function of the model intercept, $\beta_0$, and the error term, \$\epsilon\_i \$. The error term represents the unknown deviation of each observation from the population mean. Errors are closely related to **residuals**, which are the estimated errors calculated as the difference between observed values and fitted values.
...
## Quantile regression (25th percentile, median, 75th percentile)

We can also do the same for more than one quantile

```{r}
#| label: fit-rq-intercept-only-multi-tau
mod_rq_ext <- rq(lsas_post ~ 1, tau = c(0.25, 0.5, 0.75), data = df_clean)
summary(mod_rq_ext)
```

The value of the intercept is the **estimated quantile** of the outcome. Again, since this is an intercept-only model, the value is the same as these quantiles in the sample.

```{r}
#| label: sample-median-check
quantile(df_clean$lsas_post, na.rm = TRUE)
```

# Parts of the Regression Object
...
## Components of the quantile regression model

```{r}
#| label: rq-components-inspect
names(mod_rq)
coef(mod_rq)
```
...
# Getting Estimates Using `marginaleffects`
...
## Average prediction (linear regression)

```{r}
#| label: lm-avg-predictions
...
```{r}
#| label: lm-individual-predictions
predictions(mod_lm) |> head()
```

These are equal for every observation because there are no predictors.

## Median prediction (quantile regression)

```{r}
#| label: rq-avg-predictions
...
## Histogram with the estimated mean (linear model)

```{r, warning=FALSE}
#| label: plot-hist-mean-lm
...
## Estimated mean with confidence interval (linear model)

```{r}
#| label: plot-mean-ci-lm
...
## Histogram with median (quantile regression)

```{r}
#| label: plot-hist-median-rq
med_est <- coef(mod_rq)
...
## Estimated median with confidence interval (quantile regression)

```{r}
#| label: plot-median-ci-rq
...
```{r}
#| label: rq-summary-bootstrap
summary(mod_rq, se = "boot")
```

Bootstrap SEs are often more stable for quantiles.

## Using marginaleffects for CIs

### Linear regression

```{r}
#| label: lm-avg-predictions-ci

...
| Useful functions | What they do |
|------------------------------------|------------------------------------|
| `head()` | shows the *first* rows of the dataset |
| `tail()` | shows the *last* rows of the dataset |
...
2.1 Use the function to get a quick overview of the dataset and give a berif summary of it.
...
```{r}
CreateTableOne(vars=vars, data=d_bl, strata="Group")
```
...
1.  Calculate the standard error of LSAS_Screen using the code formula above, and describe in words what the number means
2.  Calculate the standard error for the proportion of men in the STePS study, and describe the meaning of this number in words
3.  What would happen to these standard errors if the sample size had been 1000 participants?
...
\
We can use this simulated sampling distribution to see how probable our *observed LSAS-SR mean* is if the (null)hypothesis that the true mean is 50 would be correct. First let plot the sampling distribution again, and show the observed LSAS-SR mean as a vertical line.
...
When the sample size increases, the t-distribution approaches the z-distribution and these estimates become very similar. A general rule of thumb is that it is okey to use z-values for sample sizes larger than 200.
:::

---
title: "Lab 4: Power Analysis and Sample Size Planning"
format: 
  html:
    crossref:
      custom:
...
#| fig-width: 10
#| fig-height: 5
#| fig-cap: "Conceptual power regions: alpha (false positive), beta (false negative), and power, with critical value lines."
...
alpha_polygon_data <- data.frame(x = x_values, y = pmin(density_null, density_alternative))
alpha_polygon_data <- alpha_polygon_data[alpha_polygon_data$x >= critical_value, ]
...
power_polygon_data <- power_polygon_data[power_polygon_data$x >= critical_value, ]
...
#| fig-cap: "Power vs sample size for d=0.5 (two-sample t-test). Dashed lines mark ~64 per group for 80% power."
...
#| fig-cap: "Power curves for small, medium, and large effects; larger effects require smaller n for the same power."
...
library(mirai)
run_power_simulation_functional <- function(
    n_per_group,
    effect_size,
    n_simulations = 1000,
    alpha = 0.05) {
...
daemons(8)
run_power_simulation_functional(
  n_per_group = sample_size_per_group,
  effect_size = 0.5,
  n_simulations = 10000
)
...
#| fig-width: 10
#| fig-height: 5
#| fig-cap: "Empirical histograms from simulations (null and alternative) overlaid on theoretical curves."
power_plot +
...
#| fig-width: 10
#| fig-height: 6
#| fig-cap: "Distribution of p-values from 10,000 Simulated Studies, showing significant results."
...
sim_results <- sim_results |>
  mutate(ci_significant = (ci_lower < 0 & ci_upper < 0) | (ci_lower > 0 & ci_upper > 0))
...
#| fig-height: 10
#| fig-width: 6
#| fig-cap: "Confidence interval coverage: green lines meet success criteria; brown/red lines do not. Vertical lines mark true effect and null effect."
...
plot_ci_coverage <- function(
  ...
  sim_data$covers_truth <- (sim_data$ci_lower <= true_effect) & (sim_data$ci_upper >= true_effect)
  ...
  ggplot(sim_data, aes(y = plot_order)) +
    geom_segment(
      aes(x = ci_lower, xend = ci_upper, color = ci_significant),
      alpha = 0.7,
      linewidth = 0.8
    ) +
    geom_vline(
    ...
    labs(
      title = title,
      subtitle = paste(
        "True effect =", true_effect, "| Power =", round(mean(sim_data$ci_significant), 3)
      ),
...
#| fig-cap: "Distribution of p-values when there is no true effect from 10,000 Simulated Studies."
#| fig-width: 10
#| fig-height: 6
#| cache: true
...
#| code-fold: true
#| cache: true
#| fig-cap: "Non-inferiority conceptual plot: one-sided alpha region and power relative to the NI margin."
...
#| fig-height: 10
#| fig-width: 6
#| fig-cap: "Non-inferiority CI visualization: green intervals exclude values worse than the margin."
...
#| tbl-cap: "Comparison of empirical power from simulation with the theoretical power for non-inferiority test."
...
#| code-fold: true
#| cache: true
#| fig-cap: "Equivalence (TOST) conceptual plot: two one-sided critical regions and central power region within ±margin."
...
density_null_pos <- dnorm(x_values, alternative_effect_size / standard_error, sigma_null)
density_alternative <- dnorm(x_values, mu_alternative, sigma_alternative)
...
power_polygon_data <- power_polygon_data[
  power_polygon_data$x >= critical_value & power_polygon_data$x <= abs(critical_value),
]
...
PALETTE <- list(
  ...
  simulations = c(
    ...
    "alternative" = "#7cecee" # TOST alternative hypothesis (effect within bounds)
  )
)
...
#| fig-height: 5
#| code-fold: true
#| fig-cap: "Empirical histograms for equivalence simulations (null bounds and alternative within bounds)."
...
#| fig-height: 10
#| code-fold: true
#| fig-cap: "Equivalence CI visualization: success when the entire 90% CI lies within ±margin."

sim_alternative_hypothesis |>
  mutate(ci_significant = ci_lower > -equivalence_bound & ci_upper < equivalence_bound) |>
  sample_n(200) |>
  plot_ci_coverage(
    true_effect = 0,
...
#| tbl-cap: "Comparison of empirical power from simulation with the theoretical TOST power."
...
sim_alternative_hypothesis |>
  summarize(
    ...
    power_ci = mean(ci_lower > -equivalence_bound & ci_upper < equivalence_bound),
    power_theoretical = theoretical_power$power
  ) |>
  pivot_longer(
    cols = everything(),

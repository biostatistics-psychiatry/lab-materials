...
Or use the `t.test()` function, and filter to using only the self-guided and therapist-guided groups
...
We can also get the confidence interval for this mean difference. As always, the confidence interval is calculated by taking the estimate of interest, in this case the mean difference $\pm$ our desired number of standard errors. For a confidence interval with large samples, the sampling distribution approaches the z-distribution (normal distribution with $\mu = 0$ and $\sigma = 1$). We can then use the z-values for the $\alpha$ covering our desired part of the distribution. To cover the middle 95% of the z-distribution, this value is $\pm 1.96$.
...
Say that we wanted to know if the gender distribution was similar across the three treatment groups in the the STEpS study. The first step to such an investigation would be to create a *contingency table* showing the gender distribution across the groups.
...
We see that there are some differences between the groups, but will we have no statistical test of these differences.

One way to test if the proportion of men and women differs between the groups would be to do a **Chi-squared test.**
...
What the test does is comparing the observed counts in each cell against what would be expected if values where distrubuted equally across the groups (i.e. the cells of the contingency table). Coding this formula manually is a but tricky, but here is the code for those interested.

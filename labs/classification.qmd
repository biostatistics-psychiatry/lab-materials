...
```{r}
lsas_fit <- logistic_reg() |>
  set_engine("glm") |>
  set_mode("classification") |>
  fit(lsas_post_bin ~ lsas_screen + group, data = d_nomiss)

lsas_pred <- predict(lsas_fit, new_data = d_nomiss) |>
  bind_cols(d_nomiss)
...
false_pos <- lsas_conf_mat$table[[2]]
...
Now that we have the confusion matrix, we can calculate the metrics of interest: sensitivity, specificity, positive predictive value, and negative predictive value.
...
Suicide is rare, PPV will always be low...
